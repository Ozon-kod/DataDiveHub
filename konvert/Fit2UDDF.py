    ...
    for record in messages:
        ...
        if record.name == 'dive_summary':
            ...
            if (decoder.fields["reference_mesg"]=="session"):
                ...
                if "avg_depth" in decoder.fields:
                    ET.SubElement(uddf_informationafterdive, "averagedepth").text = decoder.fields["avg_depth"]
                if "surface_interval" in decoder.fields:
                    ET.SubElement(uddf_informationafterdive, "surfaceinterval").text = decoder.fields["surface_interval"]
                if "bottom_time" in decoder.fields:
                    ET.SubElement(uddf_informationafterdive, "bottomtime").text = decoder.fields["bottom_time"]
                if "descent_time" in decoder.fields:
                    ET.SubElement(uddf_informationafterdive, "descenttime").text = decoder.fields["descent_time"]
                if "ascent_time" in decoder.fields:
                    ET.SubElement(uddf_informationafterdive, "ascenttime").text = decoder.fields["ascent_time"]
                if "avg_ascent_rate" in decoder.fields:
                    ET.SubElement(uddf_informationafterdive, "averageascentrate").text = decoder.fields["avg_ascent_rate"]
